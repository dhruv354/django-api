...
# from ipynb.fs.full.db_conn import *
...
# from data_visualisations import conn
# import datetime
...
def db_connect1(driver, server, database, UID, password):
    ...
    return conn
# try:
#     conn = db_connect(driver=Driver.value, server=Server.value, database=Database.value, UID=UID.value, password=password.value)
...
# print("Enter Driver value or default will be chosen")
# display(Driver)
# print("Enter Server value or defualt will be chosed")
...
def question_type_subset1(course, subject):
    ...
    # return df_final, df_final1, df_final2

# def plot_question_type(x, y, title):
print(question_type_subset1)
...
def question_heatmap():
    ...
    plt.savefig("images/question_number1.pdf")
    plt.show()
...
def question_heatmap2(courseType, questionType):
    ...
    # sns.heatmap(df_temp2.iloc[:, 20:], vmin=0, vmax=450, cmap = 'copper_r', linewidth=2, linecolor='w', annot = True, fmt = '.0f', annot_kws={'fontsize':22}, ax=ax[1])
...
def videos():
    ...
    fig.show()
    fig.write_image("images/videos1.pdf")
...
def ksc_graph():
    ...
    fig.show()
    fig.write_image("images/ksc_count1.pdf")
    ...
    fig.update_layout(uniformtext_minsize=8)
    fig.show()
    fig.write_image("images/ksc_count2.pdf")


#convert columns datetime

print(df_user_login.head())
...
def get_time_df(start_date, end_date):
    # df_user_login.createdon = pd.to_datetime(df_user_login.createdon, infer_datetime_format=True, errors='raise')
    # df_user_login.endon = pd.to_datetime(df_user_login.endon, infer_datetime_format=True, errors='raise')
    # df_user_login.lastaccessedtime = pd.to_datetime(df_user_login.lastaccessedtime, infer_datetime_format=True, errors='raise')
...
def get_time_df2(start_date, end_date):
    # df_user_login.createdon = pd.to_datetime(df_user_login.createdon, infer_datetime_format=True, errors='raise')
    # df_user_login.endon = pd.to_datetime(df_user_login.endon, infer_datetime_format=True, errors='raise')
    # df_user_login.lastaccessedtime = pd.to_datetime(df_user_login.lastaccessedtime, infer_datetime_format=True, errors='raise')
    df_user_login2 = df_user_login[(df_user_login.createdon >= start_date) & (df_user_login.lastaccessedtime <= end_date)]
    ...
    df_batch_user_course_time = pd.merge(df_batch_user_login, df_batch, on='batchid', how='left')
    df_batch_user_course_time = pd.merge(df_batch_user_course_time, df_course, left_on='courseid', right_on='COURSEID', how='left')
    return df_batch_user_course_time
...
df_user_test_session = pd.read_sql('SELECT DISTINCT COURSEID FROM USERTESTSESSION', con=conn)
df_user_test_session2 = pd.read_sql('SELECT DISTINCT SubjectId FROM USERTESTSESSION', con=conn)
df_user_test_session = pd.merge(df_user_test_session, df_course, on='COURSEID', how='left')
df_user_test_session2 = pd.merge(df_user_test_session2, df_subject_name, on='SubjectId', how='left')
df_user_test_session2.dropna(inplace=True)    
    
def question_accuracy_plot(course, subject):
    if course not in list(df_user_test_session.COURSENAME.values):
        return (True, 'this course is not present')
    if  subject not in list(df_user_test_session2.SubjectName.values):
        return (True, 'this subject is not present')
    x = df_user_test_session[df_user_test_session.COURSENAME == course]
    print(x)
    course_id = int(x.iloc[0, 0])
    x = df_user_test_session2[df_user_test_session2.SubjectName == subject]
    print(x)
    subject_id = int(x.iloc[0, 0])
    #create a dataframe of userTestSessionQuestion filtered on given course and subject
    #by joining it with  UserTestSession table using UserTestSessionId column
    print(course_id, subject_id)
    ...
    df_temp = df_user_test_session_question_accuracy[df_user_test_session_question_accuracy.new >= 20]
    return (False, df_temp)

def get_user_logged_in(start_date, end_date, freq='M'):
    ...
    df_temp.sort_values(by=['lastloggedin'], axis=0, inplace=True, ascending=True)
    df_temp['lastloggedin'] = pd.to_datetime(df_temp['lastloggedin'], infer_datetime_format=True, errors='coerce')
    df_temp['lastloggedin'] = df_temp['lastloggedin'].dt.date
    df_temp2 = df_temp.groupby(['lastloggedin'], as_index=False).agg(inactive_count=('lastloggedin', 'count'))
    df_temp2.lastloggedin = pd.to_datetime(df_temp2.lastloggedin, errors='coerce')
    df_temp2['lastloggedin'] = df_temp2['lastloggedin'].dt.date
    
    df_temp3 = df_temp2[(df_temp2.lastloggedin >= start_date) & (df_temp2.lastloggedin <= end_date)]
    print(df_temp3)
    print(df_temp3.dtypes)
    df_temp3.lastloggedin = pd.to_datetime(df_temp3.lastloggedin)
    print(df_temp3.dtypes)
    df_temp3 = df_temp3.groupby(pd.Grouper(key='lastloggedin',freq='M')).agg({'inactive_count':'sum'}).reset_index()
    return df_temp3

x = get_user_logged_in("2021-01-01",  "2021-11-30")
print(x)